...
class DoscaAPI
  def DoscaAPI.request_common(url, client_code, mail, contents_code, contents_no=nil)
    ...
    req.body[:contents_no] = if contents_no
  ...
  def DoscaAPI.update_common(url, client_code, mail, contents_code, contents_no, submit_data)
    ...
    #add submit data
    req.body[:subject] if submit_data[:subject]
    req.body[:category] if submit_data[:category]
    req.body[:summary] if submit_data[:summary]
    req.body[:web_page] if submit_data[:web_page]
    req.body[:termination_date] if submit_data[:termination_date]
    req.body[:longitude] if submit_data[:longitude]
    req.body[:latitude] if submit_data[:latitude]
    req.body[:date_time] if submit_data[:date_time]
    req.body[:cargo] if submit_data[:cargo]
    req.body[:vessel_name] if submit_data[:vessel_name]
  ...
  def DoscaAPI.pdf_download(client_code, mail, contents_code, contents_no)
    ...
    resp = http.request(req)
    return Application.symbolize_keys(JSON.parse(resp.body))
                       if resp.header.index("application/json")
  ...
  def DoscaAPI.remove(client_code, mail, contents_code, contents_no, submit_data)
    request_common(URI.parse(Settings._settings[:api][:remove_url],
